...
class MqttManager {
public:
    static constexpr uint8_t kMaxAlerts = 8;
    ...
    using AlertAckHandler = std::function<void(const String&)>;
    ...
    void setAlertAckHandler(AlertAckHandler handler);
    void publishState(
        const SystemConfig& config,
        const OutputManager& outputs,
        const LocalUiManager& localUi,
...
private:
    ...
    void publishTelemetry(
        ...
        const OutputManager& outputs,
        const TelemetrySnapshot& telemetry);
    void publishAlerts(const SystemConfig& config, const TelemetrySnapshot& telemetry);
    String topicFor(const SystemConfig& config, const char* suffix) const;

    struct PublishedAlertState {
        bool active = false;
        bool acknowledged = false;
    };
    ...
    AlertAckHandler alertAckHandler_;
    ...
    PublishedAlertState publishedAlerts_[kMaxAlerts];
};
