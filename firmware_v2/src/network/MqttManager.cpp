...
constexpr uint8_t kSupportedAlertCount = 7;

struct SupportedAlertDefinition {
    const char* code;
    MqttManager::AlertSeverity severity;
};

constexpr SupportedAlertDefinition kSupportedAlerts[kSupportedAlertCount] = {
    {"control_sensor_missing", MqttManager::AlertSeverity::Fault},
    {"control_sensor_invalid", MqttManager::AlertSeverity::Fault},
    {"control_sensor_stale", MqttManager::AlertSeverity::Fault},
    {"secondary_sensor_missing", MqttManager::AlertSeverity::Alarm},
    {"secondary_sensor_invalid", MqttManager::AlertSeverity::Alarm},
    {"secondary_sensor_stale", MqttManager::AlertSeverity::Alarm},
    {"sensor_deviation_high", MqttManager::AlertSeverity::Alarm},
};
...
const char* alertSeverityName(MqttManager::AlertSeverity severity) {
    switch (severity) {
        case MqttManager::AlertSeverity::Fault:
            return "fault";
        case MqttManager::AlertSeverity::Alarm:
            return "alarm";
        case MqttManager::AlertSeverity::Warning:
        default:
            return "warning";
    }
}

int findSupportedAlertIndex(const String& code) {
    for (uint8_t i = 0; i < kSupportedAlertCount; ++i) {
        if (code == kSupportedAlerts[i].code) {
...
const MqttManager::AlertStatus* findAlert(
    const MqttManager::TelemetrySnapshot& telemetry,
    const char* code) {
    for (uint8_t i = 0; i < telemetry.alertCount; ++i) {
        if (telemetry.alerts[i].code == code && telemetry.alerts[i].active) {
            return &telemetry.alerts[i];
        }
    }
    return nullptr;
}
...
void MqttManager::setAlertAckHandler(AlertAckHandler handler) {
    alertAckHandler_ = handler;
}
...
void MqttManager::publishState(
    ...
    publishAlerts(config, telemetry);
}
...
void MqttManager::publishAlerts(const SystemConfig& config, const TelemetrySnapshot& telemetry) {
    if (!client_.connected()) {
        return;
    }
    ...
    bool anyFault = false;
    MqttManager::AlertSeverity highestSeverity = MqttManager::AlertSeverity::Warning;
    uint8_t activeCount = 0;

    for (uint8_t index = 0; index < kSupportedAlertCount; ++index) {
        const SupportedAlertDefinition& definition = kSupportedAlerts[index];
        const AlertStatus* alert = findAlert(telemetry, definition.code);
        const bool active = alert != nullptr;
        const bool acknowledged = active && alert->acknowledged;
        const MqttManager::AlertSeverity severity = active ? alert->severity : definition.severity;
        const String message = active ? alert->message : String();

        client_.publish(
            topicFor(config, (String("alerts/") + definition.code + "/state").c_str()).c_str(),
            active ? "ON" : "OFF",
            true);
        client_.publish(
            topicFor(config, (String("alerts/") + definition.code + "/severity").c_str()).c_str(),
            alertSeverityName(severity),
            true);
        client_.publish(
            topicFor(config, (String("alerts/") + definition.code + "/message").c_str()).c_str(),
            message.c_str(),
            true);
        client_.publish(
            topicFor(config, (String("alerts/") + definition.code + "/acknowledged").c_str()).c_str(),
            acknowledged ? "ON" : "OFF",
            true);
        ...
        if (active) {
            anyActive = true;
            ++activeCount;
            if (severity == MqttManager::AlertSeverity::Fault) {
                anyFault = true;
                highestSeverity = MqttManager::AlertSeverity::Fault;
            } else if (
                highestSeverity != MqttManager::AlertSeverity::Fault
                && severity == MqttManager::AlertSeverity::Alarm) {
                highestSeverity = MqttManager::AlertSeverity::Alarm;
            }
        }

        const PublishedAlertState previous = publishedAlerts_[index];
        if (active != previous.active || (active && acknowledged && !previous.acknowledged)) {
            StaticJsonDocument<256> eventDoc;
            eventDoc["device_id"] = config.deviceId;
            eventDoc["ts"] = millis() / 1000UL;
            eventDoc["code"] = definition.code;
            eventDoc["action"] = !previous.active && active
                ? "raised"
                : (previous.active && !active ? "cleared" : "acknowledged");
            eventDoc["severity"] = alertSeverityName(severity);
            eventDoc["message"] = message;

            char eventPayload[256];
            serializeJson(eventDoc, eventPayload, sizeof(eventPayload));
            client_.publish(topicFor(config, "event/alert").c_str(), eventPayload, false);
        }

        publishedAlerts_[index].active = active;
        publishedAlerts_[index].acknowledged = acknowledged;
    }

    StaticJsonDocument<128> summaryDoc;
    summaryDoc["active"] = anyActive;
    summaryDoc["fault_active"] = anyFault;
    summaryDoc["highest_severity"] = anyActive ? alertSeverityName(highestSeverity) : "none";
    summaryDoc["active_count"] = activeCount;

    char summaryPayload[128];
    serializeJson(summaryDoc, summaryPayload, sizeof(summaryPayload));
    client_.publish(topicFor(config, "alerts/summary").c_str(), summaryPayload, true);
}
...
void MqttManager::handleFermentationConfig(const String& payload) {
    ...
    const float deviationC = alarms["deviation_c"] | NAN;
    const uint16_t sensorStaleSeconds = alarms["sensor_stale_s"] | 30;
    ...
    if (
        ...
        || isnan(deviationC) || deviationC < 0.0f || deviationC > 20.0f
        || sensorStaleSeconds < 5 || sensorStaleSeconds > 300
    ...
    updated.alarms.deviationC = deviationC;
    updated.alarms.sensorStaleSeconds = sensorStaleSeconds;
...
void MqttManager::handleCommand(const String& payload) {
    ...
    if (command == "ack_alert" && alertAckHandler_) {
        JsonObject args = doc["args"];
        const String code = args.isNull() ? String() : String(static_cast<const char*>(args["code"] | ""));
        if (!code.isEmpty()) {
            alertAckHandler_(code);
        }
        return;
    }
