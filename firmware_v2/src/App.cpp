...
constexpr const char* kControlSensorMissingCode = "control_sensor_missing";
constexpr const char* kControlSensorInvalidCode = "control_sensor_invalid";
constexpr const char* kControlSensorStaleCode = "control_sensor_stale";
constexpr const char* kSecondarySensorMissingCode = "secondary_sensor_missing";
constexpr const char* kSecondarySensorInvalidCode = "secondary_sensor_invalid";
constexpr const char* kSecondarySensorStaleCode = "secondary_sensor_stale";
constexpr const char* kSensorDeviationHighCode = "sensor_deviation_high";

uint32_t sensorStaleAfterMs(const FermentationConfig& config) {
    return static_cast<uint32_t>(config.alarms.sensorStaleSeconds) * 1000UL;
}

bool isSensorStale(const SensorManager::Reading& reading, uint32_t nowMs, const FermentationConfig& config) {
    return reading.updatedAtMs != 0
        && static_cast<uint32_t>(nowMs - reading.updatedAtMs) > sensorStaleAfterMs(config);
}
...
void addAlert(
    ...
    const char* code,
    MqttManager::AlertSeverity severity,
    const String& message,
    bool acknowledged) {
    if (telemetry.alertCount >= MqttManager::kMaxAlerts) {
        return;
    }

    MqttManager::AlertStatus& alert = telemetry.alerts[telemetry.alertCount++];
    alert.code = code;
...
void App::begin() {
    ...
    mqtt_.setAlertAckHandler([this](const String& code) { handleAlertAcknowledgement(code); });
...
void App::updateControlLoop(uint32_t nowMs) {
    ...
    if (isOtaLockoutActive()) {
        ...
        controllerChanged = controller_.update(fermentationConfig_, controllerInputs, outputs_);
    }

    const MqttManager::TelemetrySnapshot telemetry = buildTelemetrySnapshot();
    const String alertSignature = buildAlertSignature(telemetry);
    const bool alertsChanged = alertSignature != lastPublishedAlertSignature_;
    if (profileChanged || controllerChanged || alertsChanged) {
        mqtt_.publishState(config_, outputs_, localUi_, telemetry);
        lastPublishedAlertSignature_ = alertSignature;
    }
}
...
void App::handleSystemConfig(const SystemConfig& updatedConfig) {
    ...
    const MqttManager::TelemetrySnapshot telemetry = buildTelemetrySnapshot();
    mqtt_.publishState(config_, outputs_, localUi_, telemetry);
    lastPublishedAlertSignature_ = buildAlertSignature(telemetry);
...
MqttManager::TelemetrySnapshot App::buildTelemetrySnapshot() {
    MqttManager::TelemetrySnapshot telemetry;
    ...
    const uint32_t nowMs = millis();
    const bool beerProbeStale = isSensorStale(beerProbe, nowMs, fermentationConfig_);
    const bool chamberProbeStale = isSensorStale(chamberProbe, nowMs, fermentationConfig_);
    ...
    const bool secondaryEnabled = fermentationConfig_.sensors.secondaryEnabled;
    const bool useSecondaryAsControl = secondaryEnabled && fermentationConfig_.sensors.controlSensor == "secondary";
    const SensorManager::Reading& controlProbe = useSecondaryAsControl ? chamberProbe : beerProbe;
    const bool controlProbeStale = useSecondaryAsControl ? chamberProbeStale : beerProbeStale;
    const char* controlSensorLabel = useSecondaryAsControl ? "Chamber" : "Beer";
    const bool controlMissing = !controlProbe.present;
    const bool controlInvalid = controlProbe.present && !controlProbe.valid;
    const bool controlStale = controlProbe.present && controlProbeStale;

    if (controlMissing) {
        addAlert(
            telemetry,
            kControlSensorMissingCode,
            MqttManager::AlertSeverity::Fault,
            String(controlSensorLabel) + " sensor missing",
            isAlertAcknowledged(kControlSensorMissingCode));
    }
    if (controlInvalid && !controlStale) {
        addAlert(
            telemetry,
            kControlSensorInvalidCode,
            MqttManager::AlertSeverity::Fault,
            String(controlSensorLabel) + " sensor invalid",
            isAlertAcknowledged(kControlSensorInvalidCode));
    }
    if (controlStale) {
        addAlert(
            telemetry,
            kControlSensorStaleCode,
            MqttManager::AlertSeverity::Fault,
            String(controlSensorLabel) + " sensor stale",
            isAlertAcknowledged(kControlSensorStaleCode));
    }

    if (!controlMissing) {
        clearAlertAcknowledgement(kControlSensorMissingCode);
    }
    if (!(controlInvalid && !controlStale)) {
        clearAlertAcknowledgement(kControlSensorInvalidCode);
    }
    if (!controlStale) {
        clearAlertAcknowledgement(kControlSensorStaleCode);
    }

    const bool secondaryAlertScope = secondaryEnabled && fermentationConfig_.sensors.controlSensor == "primary";
    const bool secondaryMissing = secondaryAlertScope && !chamberProbe.present;
    const bool secondaryInvalid = secondaryAlertScope && chamberProbe.present && !chamberProbe.valid;
    const bool secondaryStale = secondaryAlertScope && chamberProbe.present && chamberProbeStale;
    if (secondaryMissing) {
        addAlert(
            telemetry,
            kSecondarySensorMissingCode,
            MqttManager::AlertSeverity::Alarm,
            "Chamber sensor missing",
            isAlertAcknowledged(kSecondarySensorMissingCode));
    }
    if (secondaryInvalid && !secondaryStale) {
        addAlert(
            telemetry,
            kSecondarySensorInvalidCode,
            MqttManager::AlertSeverity::Alarm,
            "Chamber sensor invalid",
            isAlertAcknowledged(kSecondarySensorInvalidCode));
    }
    if (secondaryStale) {
        addAlert(
            telemetry,
            kSecondarySensorStaleCode,
            MqttManager::AlertSeverity::Alarm,
            "Chamber sensor stale",
            isAlertAcknowledged(kSecondarySensorStaleCode));
    }

    if (!secondaryMissing) {
        clearAlertAcknowledgement(kSecondarySensorMissingCode);
    }
    if (!(secondaryInvalid && !secondaryStale)) {
        clearAlertAcknowledgement(kSecondarySensorInvalidCode);
    }
    if (!secondaryStale) {
        clearAlertAcknowledgement(kSecondarySensorStaleCode);
    }

    const bool beerProbeUsable = beerProbe.valid && !beerProbeStale;
    const bool chamberProbeUsable = chamberProbe.valid && !chamberProbeStale;
    const bool deviationHigh = secondaryEnabled && beerProbeUsable && chamberProbeUsable
        && fabsf(
            (beerProbe.tempC + fermentationConfig_.sensors.primaryOffsetC)
            - (chamberProbe.tempC + fermentationConfig_.sensors.secondaryOffsetC))
            > fermentationConfig_.alarms.deviationC;
    if (deviationHigh) {
        addAlert(
            telemetry,
            kSensorDeviationHighCode,
            MqttManager::AlertSeverity::Alarm,
            "Beer and chamber sensors deviate",
            isAlertAcknowledged(kSensorDeviationHighCode));
    } else {
        clearAlertAcknowledgement(kSensorDeviationHighCode);
    }
...
ControllerEngine::Inputs App::buildControllerInputs() const {
    ...
    const SensorManager::Reading& chamberProbe = sensors_.chamberProbe();
    inputs.nowMs = millis();
    const bool beerProbeStale = isSensorStale(beerProbe, inputs.nowMs, fermentationConfig_);
    const bool chamberProbeStale = isSensorStale(chamberProbe, inputs.nowMs, fermentationConfig_);
...
FermentationConfig App::buildDefaultFermentationConfig(const String& deviceId) const {
    ...
    config.alarms.deviationC = 2.0f;
    config.alarms.sensorStaleSeconds = 30;
    return config;
}

void App::handleAlertAcknowledgement(const String& code) {
    if (!acknowledgeAlertCode(code)) {
        return;
    }
...
String App::buildAlertSignature(const MqttManager::TelemetrySnapshot& telemetry) const {
    String signature;
    for (uint8_t i = 0; i < telemetry.alertCount; ++i) {
        const MqttManager::AlertStatus& alert = telemetry.alerts[i];
        signature += alert.code;
        signature += ':';
        signature += alert.acknowledged ? '1' : '0';
        signature += ':';
...
bool App::acknowledgeAlertCode(const String& code) {
    if (code == kControlSensorMissingCode) {
        ackControlSensorMissing_ = true;
        return true;
    }
    if (code == kControlSensorInvalidCode) {
        ackControlSensorInvalid_ = true;
        return true;
    }
    if (code == kControlSensorStaleCode) {
        ackControlSensorStale_ = true;
        return true;
    }
    if (code == kSecondarySensorMissingCode) {
        ackSecondarySensorMissing_ = true;
        return true;
    }
    if (code == kSecondarySensorInvalidCode) {
        ackSecondarySensorInvalid_ = true;
        return true;
    }
    if (code == kSecondarySensorStaleCode) {
        ackSecondarySensorStale_ = true;
        return true;
    }
    if (code == kSensorDeviationHighCode) {
        ackSensorDeviationHigh_ = true;
        return true;
    }
    return false;
}

bool App::isAlertAcknowledged(const char* code) const {
    if (strcmp(code, kControlSensorMissingCode) == 0) {
        return ackControlSensorMissing_;
    }
    if (strcmp(code, kControlSensorInvalidCode) == 0) {
        return ackControlSensorInvalid_;
    }
    if (strcmp(code, kControlSensorStaleCode) == 0) {
        return ackControlSensorStale_;
    }
    if (strcmp(code, kSecondarySensorMissingCode) == 0) {
        return ackSecondarySensorMissing_;
    }
    if (strcmp(code, kSecondarySensorInvalidCode) == 0) {
        return ackSecondarySensorInvalid_;
    }
    if (strcmp(code, kSecondarySensorStaleCode) == 0) {
        return ackSecondarySensorStale_;
    }
    if (strcmp(code, kSensorDeviationHighCode) == 0) {
        return ackSensorDeviationHigh_;
    }
    return false;
}

void App::clearAlertAcknowledgement(const char* code) {
    if (strcmp(code, kControlSensorMissingCode) == 0) {
        ackControlSensorMissing_ = false;
        return;
    }
    if (strcmp(code, kControlSensorInvalidCode) == 0) {
        ackControlSensorInvalid_ = false;
        return;
    }
    if (strcmp(code, kControlSensorStaleCode) == 0) {
        ackControlSensorStale_ = false;
        return;
    }
    if (strcmp(code, kSecondarySensorMissingCode) == 0) {
        ackSecondarySensorMissing_ = false;
        return;
    }
    if (strcmp(code, kSecondarySensorInvalidCode) == 0) {
        ackSecondarySensorInvalid_ = false;
        return;
    }
    if (strcmp(code, kSecondarySensorStaleCode) == 0) {
        ackSecondarySensorStale_ = false;
        return;
    }
    if (strcmp(code, kSensorDeviationHighCode) == 0) {
        ackSensorDeviationHigh_ = false;
    }
}
